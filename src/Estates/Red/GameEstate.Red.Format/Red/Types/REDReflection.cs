...
namespace GameEstate.Red.Formats.Red.Types
{
    ...
    public static class REDReflection
    {
        public static string GetREDNameString(Member item)
        {
            ...
            return attribute is null || string.IsNullOrWhiteSpace(attribute.Name) ? item.Name : attribute.Name;
        }

        public static string GetREDTypeString(Type type, params int[] flags) => GetREDTypeString(type, flags.AsEnumerable().GetEnumerator());

        static string GetREDTypeString(Type type, IEnumerator<int> flags)
        {
        ...
        static string GetREDTypeFroWkitType(string typename)
        {
            switch (typename)
            {
        ...
        static IEnumerable<Member> GetREDBuffersInternal(this CVariable cvar)
        {
            // get only REDBuffers
        ...
        static IEnumerable<Member> GetREDMembersInternal(this CVariable cvar, bool getBuffers)
        {
