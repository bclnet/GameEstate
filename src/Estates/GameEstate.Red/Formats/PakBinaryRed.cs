                ...
                case KEY_MAGIC: // Signature("KEY ")
                    {
                        ...
                        for (var i = 0; i < header.NumFiles; i++)
                        {
                            ...
                            files[i] = new FileMetadata
                            {
                                ...
                                Pak = new SubPakFile(source.Estate, source.Game, subPath, (headerKeys, (uint)i)),
                            };
                        }
                    }
                ...
                case BUNDLE_MAGIC: // Signature("POTATO70")
                    {
                        ...
                        {
                            var headerFile = headerFiles[i];
                            var path = UnsafeX.ReadZASCII(headerFile.Name, 0x100).Replace('\\', '/');
                            files[i] = new FileMetadata
                            {
                                Path = path,
                                ObjectFactory = ObjectFactory(path),
