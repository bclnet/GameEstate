...
namespace GameEstate.Cry.Formats
{
    ...
    public static class CryXmlSerializer
    {
        ...
        public static XmlDocument ReadStream(Stream inStream, bool writeLog = false)
        {
            ...
            if (header != "CryXmlB") throw new Exception("Unknown File Format");
            ...
            {
                var position = r.BaseStream.Position;
                var value = new CryXmlReference { NameOffset = r.ReadInt32(), ValueOffset = r.ReadInt32() };
                attributeTable.Add(value);
                if (writeLog) Console.WriteLine("0x{0:X6}: {1:X8} {2:X8}", position, value.NameOffset, value.ValueOffset);
            }
            ...
            while (r.BaseStream.Position < offset3 + count3 * length3)
            {
                ...
                table3.Add(value);
                if (writeLog) Console.WriteLine("0x{0:X6}: {1:X8}", position, value);
            }
            ...
            {
                var position = r.BaseStream.Position;
                var value = new CryXmlValue { Offset = (int)position - contentOffset, Value = r.ReadCString() };
                dataTable.Add(value);
                if (writeLog) Console.WriteLine("0x{0:X6}: {1:X8} {2}", position, value.Offset, value.Value);
            }
            ...
            foreach (var node in nodeTable)
            {
                ...
                for (int i = 0, j = node.AttributeCount; i < j; i++)
                {
                    ...
                    else { /*bugged = true;*/ element.SetAttribute(dataMap[attributeTable[attributeIndex].NameOffset], "BUGGED"); }
                    attributeIndex++;
                }
