...
namespace GameEstate.Cry.Formats
{
    public abstract class CryObjectWriter
    {
        ...
        protected string GetOutputFile(string extension, string outputDir = null, bool preservePath = true)
        {
            ...
            if (string.IsNullOrWhiteSpace(outputDir)) outputFile = Path.Combine(new FileInfo(CryFile.InputFile).DirectoryName, $"{Path.GetFileNameWithoutExtension(CryFile.InputFile)}{(NoConflicts ? "_out" : string.Empty)}.{extension}");
            else
            {
