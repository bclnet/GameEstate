...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class P4kFile : ZipFile
	{
		...
		public new Stream GetInputStream(long entryIndex)
		{
			...
			Stream result = (Stream)Activator.CreateInstance(PartialInputStreamType, this, start, entries_[entryIndex].CompressedSize);
			...
			if (entries_[entryIndex] is P4kEntry entry && entry.IsAesCrypted == true)
			{
				...
				while (testing && (entryIndex < Count))
				{
					...
					if (testing && testData && this[entryIndex].IsFile)
					{
						...
						if ((this[entryIndex].Flags & (int)GeneralBitFlags.Descriptor) != 0)
						{
							...
							ZipFormat_ReadDataDescriptor(baseStream_, this[entryIndex].LocalHeaderRequiresZip64, data);
