...
namespace GameEstate.Red.Formats.Red.CR2W
{
    public class CR2WFile : ObservableObject, IWolvenkitFile
    {
        ...
        const long MAGIC_SIZE = 4;
        const long FILEHEADER_SIZE = 36;
        const long TABLEHEADER_SIZE = 12 * 10;
        readonly int[] TABLES_SIZES = new int[6] { 8, 8, 16, 24, 24, 24 };
        ...
        public CR2WFile()
        {
            ...
            _fileheader = new CR2WFileHeader()
            {
                version = 162,
            };
            ...
            _tableheaders = new CR2WTable[10];
        }

        // constants
        const uint MAGIC = 0x57325243; // "W2RC"
        const uint DEADBEEF = 0xDEADBEEF;

        // IO
        CR2WFileHeader _fileheader;
        CR2WTable[] _tableheaders;
        byte[] m_strings;
        ...
        CR2WFile additionalCr2WFile;
        byte[] additionalCr2WFileBytes;
        ...
        public int GetLastChildrenIndexRecursive(CR2WExportWrapper chunk) => chunk.VirtualChildrenChunks.Count() == 0
            ? chunk.ChunkIndex
            : GetLastChildrenIndexRecursive(chunk.VirtualChildrenChunks.Where(_ => _.ChunkIndex == chunk.VirtualChildrenChunks.Max(p => p.ChunkIndex)).FirstOrDefault());
        ...
        public CR2WFileHeader GetFileHeader() => _fileheader;
        public CR2WTable[] GetTableHeaders() => _tableheaders;
        ...
        public CVariable ReadVariable(BinaryReader file, CVariable parent)
        {
            ...
            //}
            //catch { //    System.Console.WriteLine("hohoho"); }
        ...
        public (List<CR2WImportWrapper>, bool, List<CR2WBufferWrapper>) ReadImportsAndBuffers(BinaryReader r)
        {
            // read file header
            var id = r.BaseStream.ReadStruct<uint>();
            if (id != MAGIC)
                throw new FormatException($"Not a CR2W file, Magic read as 0x{id:X8}");

            _fileheader = r.BaseStream.ReadStruct<CR2WFileHeader>();

            //TODO: CP77
            if (_fileheader.version > 195 || _fileheader.version < 163)
                throw new FormatException($"Unknown Version {_fileheader.version}. Supported versions: 159 - 163.");

            var dt = new CDateTime(_fileheader.timeStamp, null, "");

            _tableheaders = r.BaseStream.ReadStructs<CR2WTable>(10);
            m_hasInternalBuffer = _fileheader.bufferSize > _fileheader.fileSize;

            // read strings
            m_strings = ReadStringsBuffer(r.BaseStream);

            // read tables
            Names = ReadTable<CR2WName>(r.BaseStream, 1).Select(_ => new CR2WNameWrapper(_, this)).ToList();
            Imports = ReadTable<CR2WImport>(r.BaseStream, 2).Select(_ => new CR2WImportWrapper(_, this)).ToList();
            Properties = ReadTable<CR2WProperty>(r.BaseStream, 3).Select(_ => new CR2WPropertyWrapper(_)).ToList();
            Chunks = ReadTable<CR2WExport>(r.BaseStream, 4).Select(_ => new CR2WExportWrapper(_, this)).ToList();
            Buffers = ReadTable<CR2WBuffer>(r.BaseStream, 5).Select(_ => new CR2WBufferWrapper(_)).ToList();
            Embedded = ReadTable<CR2WEmbedded>(r.BaseStream, 6).Select(_ => new CR2WEmbeddedWrapper(_)
            {
                ParentFile = this,
        ...
        public async Task<EFileReadErrorCodes> Read(BinaryReader file)
        {
            ...
            _fileheader = file.BaseStream.ReadStruct<CR2WFileHeader>();
            if (_fileheader.version > 195 || _fileheader.version < 163)
            ...
            _tableheaders = file.BaseStream.ReadStructs<CR2WTable>(10);
            m_hasInternalBuffer = _fileheader.bufferSize > _fileheader.fileSize;
        ...
        T[] ReadTable<T>(Stream stream, int index) where T : struct
        {
            //stream.Seek(m_tableheaders[index].offset, SeekOrigin.Begin);
            var hash = new Crc32Algorithm(false);
            var table = stream.ReadStructs<T>(_tableheaders[index].itemCount, hash);
            return table;
        }

        byte[] ReadStringsBuffer(Stream stream)
        {
            var start = _tableheaders[0].offset;
            var m_strings_size = _tableheaders[0].itemCount;
            var crc = _tableheaders[0].crc32;
            var _temp = new byte[m_strings_size];
            stream.Read(_temp, 0, _temp.Length);
            var offset = 0U;
            var tempstring = new List<byte>();
            for (var i = 0U; i < m_strings_size; i++)
            {
                var b = _temp[i];
                if (b == 0)
                {
        ...
        public void Write(BinaryWriter file)
        {
            ...
            _tableheaders[0].offset = stringbuffer_offset;
            m_strings = newstrings.ToArray();

            _tableheaders[0].itemCount = (uint)m_strings.Length;
            _tableheaders[0].crc32 = Crc32Algorithm.Compute(m_strings);
            ...
            _fileheader.fileSize += headerOffset;
            _fileheader.bufferSize += headerOffset;
            #endregion
            ...
            WriteHeader(file);
            _fileheader.crc32 = CalculateHeaderCRC32();
            ...
            uint CalculateHeaderCRC32()
            {
                ...
                hash.Append(BitConverter.GetBytes(_fileheader.version));
                hash.Append(BitConverter.GetBytes(_fileheader.flags));
                hash.Append(BitConverter.GetBytes(_fileheader.timeStamp));
                hash.Append(BitConverter.GetBytes(_fileheader.buildVersion));
                hash.Append(BitConverter.GetBytes(_fileheader.fileSize));
                hash.Append(BitConverter.GetBytes(_fileheader.bufferSize));
                hash.Append(BitConverter.GetBytes(DEADBEEF));
                hash.Append(BitConverter.GetBytes(_fileheader.numChunks));
                foreach (var h in _tableheaders)
                {
        ...
        private void WriteHeader(BinaryWriter file)
        {
            ...
            #region Write Tables
            _tableheaders[1].itemCount = (uint)Names.Count;
            _tableheaders[1].offset = (uint)file.BaseStream.Position;
            WriteTable<CR2WName>(file.BaseStream, Names.Select(_ => _.Name).ToArray(), 1);

            _tableheaders[2].itemCount = (uint)Imports.Count;
            _tableheaders[2].offset = Imports.Count > 0 ? (uint)file.BaseStream.Position : 0;
            WriteTable<CR2WImport>(file.BaseStream, Imports.Select(_ => _.Import).ToArray(), 2);

            _tableheaders[3].itemCount = (uint)Properties.Count;
            _tableheaders[3].offset = (uint)file.BaseStream.Position;
            WriteTable<CR2WProperty>(file.BaseStream, Properties.Select(_ => _.Property).ToArray(), 3);

            _tableheaders[4].itemCount = (uint)Chunks.Count;
            _tableheaders[4].offset = (uint)file.BaseStream.Position;
            WriteTable<CR2WExport>(file.BaseStream, Chunks.Select(_ => _.Export).ToArray(), 4);

            if (Buffers.Count > 0)
            {
                _tableheaders[5].itemCount = (uint)Buffers.Count;
                _tableheaders[5].offset = (uint)file.BaseStream.Position;
                WriteTable<CR2WBuffer>(file.BaseStream, Buffers.Select(_ => _.Buffer).ToArray(), 5);
            }

            if (Embedded.Count > 0)
            {
                _tableheaders[6].itemCount = (uint)Embedded.Count;
                _tableheaders[6].offset = (uint)file.BaseStream.Position;
        ...
        private void WriteFileHeader(BinaryWriter file)
        {
            ...
            file.BaseStream.WriteStruct<CR2WFileHeader>(_fileheader);
            file.BaseStream.WriteStructs<CR2WTable>(_tableheaders); // offsets change if stringtable changes
        ...
        private void WriteData(BinaryWriter bw)
        {
            ...
            _fileheader.fileSize = (uint)bw.BaseStream.Position;
            ...
            _fileheader.bufferSize = (uint)bw.BaseStream.Position;
        ...
        private void WriteTable<T>(Stream stream, T[] array, int index) where T : struct
        {
            ...
            _tableheaders[index].crc32 = crc.HashUInt32;
        }
