...
namespace P4KLib
{
    public class P4KFile
    {
        ...
        {
            get => centralDirectory.Filename;
            set
            {
                if (Path.IsPathRooted(value)) throw new Exception("Paths can't be rooted in the P4K Virtual File System");
        ...
        public P4KFile(P4KVFS filesystem, string filename, byte[] data)
        {
            ...
            if (filesystem.FilepathExists(filename)) throw new Exception("File already exists");
        ...
        public FileStructure FileStructure
        {
            get
            {
                ...
                {
                    var mutex = Filesystem.GetStream(out var stream, out var reader, out var writer);
        ...
        public override string ToString()
            => $"{Path.GetFileName(centralDirectory.Filename)} [{FileStructure.Extra.CompressedFileLength}b]";
        ...
        public byte[] GetData(bool decrypt = true)
        {
            ...
            var isEqual = Enumerable.SequenceEqual(dataHash, centralDirectory.Extra.sha256_hash);
            if (!isEqual) throw new Exception("SHA256 integrity check failed");
            ...
            switch (_fileStructure.CompressionMode)
            {
                case FileStructure.FileCompressionMode.Uncompressed: break;
                case FileStructure.FileCompressionMode.ZStd: data = UnmanagedZStd.ZStd.StreamingUncompress(data, (ulong)this.centralDirectory.Extra.uncompressed_file_length); break;
                default: throw new NotImplementedException();
            }
            return data;
        }

        void SetData() { throw new NotImplementedException(); }
    }
}
