...
namespace P4KLib
{
    ...
    public class Crc32Algorithm : HashAlgorithm
    {
        ...
        public static uint Append(uint initial, byte[] input, int offset, int length)
        {
            ...
            if (offset < 0 || length < 0 || offset + length > input.Length) throw new ArgumentOutOfRangeException("length");
        ...
        public static bool IsValidWithCrcAtEnd(byte[] input, int offset, int lengthWithCrc)
            => Append(0, input, offset, lengthWithCrc) == 0x2144DF1C;
        ...
        {
            if (_isBigEndian) return new[] { (byte)(_currentCrc >> 24), (byte)(_currentCrc >> 16), (byte)(_currentCrc >> 8), (byte)_currentCrc };
            else return new[] { (byte)_currentCrc, (byte)(_currentCrc >> 8), (byte)(_currentCrc >> 16), (byte)(_currentCrc >> 24) };
        }
        ...
        {
            if (length > 0) return _proxy.Append(initial, input, offset, length);
            else return initial;
        }
    }
}
