...
namespace P4KLib
{
    public class FileStructure : IPackageStructure
    {
        ...
        public FileStructure(Stream s, BinaryReader r)
        {
            if ((s.Position - 4) % 0x1000 != 0) throw new Exception("File Data Section Alignment Error (Pre Read)");

            Version = r.ReadInt16();
            Flags = r.ReadInt16();
            CompressionMode = (FileCompressionMode)r.ReadInt16();
            ModificationTime = r.ReadUInt16();
            ModificationDate = r.ReadUInt16();
            CRC32 = r.ReadInt32();
            CompressedSize = r.ReadInt32();
            UncompressedSize = r.ReadInt32();
            FilenameLength = r.ReadInt16();
            ExtraLength = r.ReadInt16();

            Filename = r.ReadString(FilenameLength);
            Extra = new FileStructureExtraData(s, r, ExtraLength);
        }
        ...
        public class FileStructureExtraData : IPackageStructureExtra
        {
            ...
            public FileStructureExtraData(Stream stream, BinaryReader reader, int extra_length)
            {
                ...
                if (stream.Position % 0x1000 != 0) throw new Exception("File Data Section Alignment Error (End Extra Read)");
